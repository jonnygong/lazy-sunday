<template>
    <section>
        ...
        <el-table :data="list"
            ...
            <el-table-column prop="type_id" label="所属分类" width="120">
              <template scope="scope">
                {{ options.type[scope.row.type_id] }}
              </template>
            </el-table-column>
            <!-- 普通列表显示 -->
            <el-table-column
            ...
            <el-table-column prop="is_top" label="是否置顶" width="120">
              <template scope="scope">
                {{ options.is_top[scope.row.is_top].label }}
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      async getListType() {
        ...
        res.data.forEach(item => {
          this.options.type[item.id] = item.type
        });
      ...
      async getListArea() {
        ...
        res.data.forEach(item => {
          this.options.area[item.id] = item.area_name
        });
        this.getListData();
        this.getListType();
      },
      ...
      handleDel(index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          const res = await this.$http.post(`articleDelete`, params);
          if (res === null) return;
          this.$message({
            message: '状态修改成功',
            type: 'success'
