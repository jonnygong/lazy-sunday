<template>
    <section>
        ...
        <el-table :data="list"
            ...
            <!-- 图片显示 -->
            <!-- <el-table-column prop="icon" label="分类图标" min-width="130">
                <template scope="scope">
                    <el-popover trigger="hover" placement="top">
                        <div class="ad-img">
                            <img :src="scope.row.icon" :alt="scope.row.name" width="200" height="auto"
                                 v-if="scope.row.icon !== ''">
                            <p v-else>暂无图片</p>
                        </div>
  ...
  export default {
    ...
    methods: {
      ...
      async getListData() {
        ...
        const res = await this.$http.get(`typeList`, params);
      ...
      handleDel(index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          const res = await this.$http.post(`typeDelete`, params);
          if (res === null) return;
          this.$message({
            message: '状态修改成功',
            type: 'success'
