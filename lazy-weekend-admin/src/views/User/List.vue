    ...
    <section>
        ...
        <el-table :data="list"
           ...
           <el-table-column prop="sex" label="性别" width="80">
            ...
            <!-- 图片显示 -->
            <el-table-column prop="cover" label="用户头像" width="130">
                <template scope="scope">
            ...
            <el-table-column prop="life_status" label="生活状态" width="120">
              <template scope="scope">
                {{ life_status[scope.row.life_status] }}
              </template>
            </el-table-column>
             <!-- 时间戳转日期 -->
    ...
    methods: {
      ...
      async getListData() {
        ...
        let params = {
           page: this.page,
          size: this.pagesize,
          keyword: this.filters.value, // 可选参数查询
          value: this.filters.key // 可选参数查询
        };
        const res = await this.$http.get(`userList`, params);
        this.listLoading = false;
        if (res === null) return;
        this.list = res.data.list;
        this.total = res.data.count;
        this.options.sex.forEach(item => {
          this.sex[item.value] = item.label;
        });
        this.options.life_status.forEach(item => {
          this.life_status[item.value] = item.label;
        });
      },
      //删除
      handleDel(index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          const res = await this.$http.post(`userDelete`, params);
          if (res === null) return;
          this.$message({
            message: '状态修改成功',
            type: 'success'
